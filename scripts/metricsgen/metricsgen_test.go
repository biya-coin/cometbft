...
import (
	...
	metricsgen "github.com/biya-coin/cometbft/scripts/metricsgen" //nolint:revive // this only works with metricsgen in front
)
...
func TestParseAliasedMetric(t *testing.T) {
	aliasedData := `
			...
			import(
				mymetrics "github.com/biya-coin/cometbft/libs/metrics"
			)
			type Metrics struct {
				m mymetrics.Gauge
			}
